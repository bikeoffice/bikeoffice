...
import { Create, DateInput, SimpleForm, TextInput, required, ReferenceInput, SelectInput, FormDataConsumer, useNotify, useRedirect, useDataProvider } from "react-admin";
	...
	return (
		<Create {...props}>
			<SimpleForm onSubmit={handleSave}>
				...
				<SelectInput value={sizeId} onChange={event => setSizeId(event.target.value)} source="sizeId" label="Size" choices={sizes} optionText="name" validate={required()} />
				<SelectInput source="bikeId" label="Bike" disabled={sizeId == undefined} choices={availableBikes} optionText="name" validate={required()} />
				<FormDataConsumer>
					{({ formData, ...rest }) => (
						<>
							<SelectInput
							...
							{formData.clientOption === "existing" ? (
								<ReferenceInput label="Client" source="clientId" reference="clients" validate={required()} perPage={100} sort={{ field: "name", order: "ASC" }}>
