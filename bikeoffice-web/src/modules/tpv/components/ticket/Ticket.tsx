...
export const Ticket = forwardRef(({ closeTicket }: any, ref) => {
	...
	const getIva = useCallback(
		() => selectedProducts && Array.from(selectedProducts).reduce((a, [, v]) => a.set(v.iva, (a.get(v.iva) || 0) + v.price * v.quantity), new Map<number, number>()),
		[selectedProducts],
	);
	...
	return (
		<section aria-label="Ticket">
			...
			<button className="close" onClick={handleCloseTicket}>
				Cobrar
			</button>
		</section>
	);
