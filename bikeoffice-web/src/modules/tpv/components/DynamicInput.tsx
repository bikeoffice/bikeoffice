...
export const DynamicInput: React.FC<DynamicInputProps> = ({ name, id, value, setValue, afterChange, minWidth, maxValue, growth }) => {
    ...
    return (
        <span id={name}>
            <input type="text" name={name} value={toInt(value) ?? "0"} onChange={onChange} onBlur={(e) => afterChange && afterChange(id, e.target.value)} style={{ width: (growth ?? 0) * (value?.toString().length ?? 0) + (minWidth ?? 30) }} />
        </span>
    )
}
